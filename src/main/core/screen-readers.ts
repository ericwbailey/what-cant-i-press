...
// ---------------------------------------------------------------------------
//
// The "NA" token is the Narrator key (Caps Lock or Insert by default,
// configurable), rendered as its own chip. It is a compact abbreviation, like
// VoiceOver's "VO" key. "Plus"/"Minus" are the physical +/- keys pressed without
// Shift (per the manual's note); numeric-keypad variants use "Num Pad Plus/Minus/5".
const NARRATOR_COMMANDS: ReaderCommand[] = [
  // General
  cmd('NA + Escape', 'Exit Narrator'),
  cmd('NA + 1', 'Toggle input learning'),
  cmd('NA + Right Arrow', 'Move to next item'),
  cmd('NA + Left Arrow', 'Move to previous item'),
  cmd('NA + Page Up', 'Change view'),
  cmd('Ctrl + NA + Up Arrow', 'Change view'),
  cmd('NA + Page Down', 'Change view'),
  cmd('Ctrl + NA + Down Arrow', 'Change view'),
  cmd('NA + F1', 'Show commands list'),
  cmd('NA + F2', 'Show commands for current item'),
  cmd('NA + Enter', 'Do primary action'),
  cmd('NA + Ctrl + Enter', 'Toggle search mode'),
  cmd('NA + Backslash', 'Read the status bar in apps such as Word, Excel, and PowerPoint'),
  cmd('NA + F12', 'Read current time and date'),
  cmd('NA + Shift + B', 'Read Battery and Network status'),
  cmd('NA + Alt + B', 'Toggle braille viewer'),
  cmd('NA + Ctrl + C', 'Toggle Screen curtain'),
  cmd('NA + Ctrl + D', 'Describe image using an online service or get the webpage source of a link'),
  cmd('NA + S', 'Get a webpage summary'),
  cmd('NA + S twice quickly', 'Get webpage summary and popular links dialog box'),
  cmd('NA + Shift + S', 'Speech off'),
  cmd('NA + Alt + F', 'Provide Narrator feedback'),
  cmd('NA + Z', 'Lock Narrator key'),
  cmd('NA + Ctrl + F12', 'Toggle developer mode'),
  cmd('NA + 3', 'Pass keys to application'),
  cmd('NA + 4', 'Change capitalization reading mode'),
  cmd('NA + Alt + M', 'Toggle mouse mode'),
  cmd('NA + H', 'Turn on or off Outlook column header reading'),

  // Adjust speech
  cmd('Ctrl + NA + Plus', 'Increase voice volume'),
  cmd('Ctrl + NA + Num Pad Plus', 'Increase voice volume'),
  cmd('Ctrl + NA + Minus', 'Decrease voice volume'),
  cmd('Ctrl + NA + Num Pad Minus', 'Decrease voice volume'),
  cmd('NA + Plus', 'Increase voice speed'),
  cmd('NA + Minus', 'Decrease voice speed'),
  cmd('NA + Alt + Plus', 'Move to the next voice'),
  cmd('NA + Alt + Num Pad Plus', 'Move to the next voice'),
  cmd('NA + Alt + Minus', 'Move to the previous voice'),
  cmd('NA + Alt + Num Pad Minus', 'Move to the previous voice'),
  cmd('NA + Alt + Left Bracket', 'Change to the prior punctuation reading mode'),
  cmd('NA + Alt + Right Bracket', 'Change to the next punctuation reading mode'),
  cmd('NA + V', 'Increase verbosity mode'),
  cmd('NA + Shift + V', 'Decrease verbosity mode'),
  cmd('NA + 2', 'Toggle character reading'),
  cmd('NA + Slash', 'Read context'),
  cmd('NA + Alt + Slash', 'Set read context verbosity'),
  cmd('NA + Ctrl + Slash', 'Change read context order'),

  // Read and work with text
  cmd('NA + Tab', 'Read item'),
  cmd('NA + Num Pad 5', 'Read item'),
  cmd('NA + Tab twice quickly', 'Read item spelled out'),
  cmd('NA + Num Pad 5 twice quickly', 'Read item spelled out'),
  cmd('NA + K twice quickly', 'Read item spelled out'),
  cmd('NA + Ctrl + Num Pad 5 twice quickly', 'Read item spelled out'),
  cmd('NA + 0', 'Read item advanced'),
  cmd('NA + T', 'Read window title'),
  cmd('NA + W', 'Read window'),
  cmd('NA + X', 'Re-hear what Narrator spoke last'),
  cmd('NA + Ctrl + X', 'Copy last spoken phrase to clipboard'),
  cmd('NA + Alt + X', 'Open speech recap window for history and live transcription'),
  cmd('NA + R', 'Read from cursor'),
  cmd('Ctrl + NA + R', 'Start reading document'),
  cmd('NA + Down Arrow', 'Start reading document'),
  cmd('NA + C', 'Read document'),
  cmd('NA + Shift + J', 'Read text from start to cursor'),
  cmd('NA + Alt + Home', 'Read text from start to cursor'),
  cmd('Ctrl + NA + U', 'Read previous page'),
  cmd('Ctrl + NA + I', 'Read current page'),
  cmd('Ctrl + NA + O', 'Read next page'),
  cmd('Ctrl + NA + J', 'Read previous paragraph'),
  cmd('Ctrl + NA + K', 'Read current paragraph'),
  cmd('Ctrl + NA + L', 'Read next paragraph'),
  cmd('NA + Ctrl + M', 'Read previous sentence'),
  cmd('NA + Ctrl + Comma', 'Read current sentence'),
  cmd('NA + Ctrl + Period', 'Read next sentence'),
  cmd('NA + U', 'Read previous line'),
  cmd('NA + I', 'Read current line'),
  cmd('NA + Up Arrow', 'Read current line'),
  cmd('NA + O', 'Read next line'),
  cmd('NA + J', 'Read previous word'),
  cmd('Ctrl + NA + Left Arrow', 'Read previous word'),
  cmd('NA + K', 'Read current word'),
  cmd('Ctrl + NA + Num Pad 5', 'Read current word'),
  cmd('NA + L', 'Read next word'),
  cmd('Ctrl + NA + Right Arrow', 'Read next word'),
  cmd('NA + M', 'Read previous character'),
  cmd('NA + Comma', 'Read current character'),
  cmd('Num Pad 5', 'Read current character'),
  cmd('NA + Period', 'Read next character'),
  cmd('NA + F', 'Read next group of formatting information'),
  cmd('NA + Shift + F', 'Read previous group of formatting information'),
  cmd('NA + B', 'Move to beginning of text'),
  cmd('Ctrl + NA + Home', 'Move to beginning of text'),
  cmd('NA + E', 'Move to end of text'),
  cmd('Ctrl + NA + End', 'Move to end of text'),
  cmd('NA + Shift + Down Arrow', 'Read selection'),
  cmd('NA + Shift + Down Arrow twice quickly', 'Spell selection'),
  ...
  cmd('NA + Home', 'Move to first item in window'),
  cmd('NA + End', 'Move to last item in window'),
  cmd('NA + Backspace', 'Go back one item'),
  cmd('NA + N', 'Move to main landmark'),
  cmd('NA + Left Bracket', 'Move Narrator cursor to system cursor'),
  cmd('NA + Num Pad Minus', 'Move Narrator cursor to system cursor'),
  cmd('NA + Apostrophe', 'Set focus to item'),
  cmd('NA + Num Pad Plus', 'Set focus to item'),
  cmd('NA + A', 'Jump to linked item'),
  cmd('NA + Shift + A', 'Jump to annotated content'),
  cmd('NA + Alt + Up Arrow', 'Navigate to parent (when structural navigation is provided)'),
  cmd('NA + Alt + Right Arrow', 'Navigate to next sibling (when structural navigation is provided)'),
  cmd('NA + Alt + Left Arrow', 'Navigate to previous sibling (when structural navigation is provided)'),
  cmd('NA + Alt + Down Arrow', 'Navigate to first child (when structural navigation is provided)'),
  cmd('NA + F7', 'List of links'),
  cmd('NA + F5', 'List of landmarks'),
  cmd('NA + F6', 'List of headings'),
  cmd('NA + Ctrl + F', 'Narrator Find'),
  cmd('NA + F3', 'Continue Find forward'),
  cmd('NA + Shift + F3', 'Continue Find backward')
]
